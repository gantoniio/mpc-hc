...
void srt_header(char (&outBuffer)[1024], const STSStyle& style, const SubRendererSettings& subRendererSettings) {
    ...
    CString fontName = style.fontName;
    CStringA langTagStr = "";
    if (subRendererSettings.openTypeLangHint[0]) {
        ...
        //hack to make sure opentypelanghint can actually work with a supported font.
        //Tahoma and Arial are common defaults so substitute Calibri (newer Windows default font) to support hinting
        if (fontName == _T("Tahoma") || fontName == _T("Arial")) {
            fontName = _T("Calibri");
        }
    }
    CT2CA tmpFontName(fontName);
